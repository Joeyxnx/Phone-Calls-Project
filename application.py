...
def process_event_history(log: dict[str, list[dict]],
    ...
    for item in log["events"]:
        month = datetime.datetime.strptime(item['time'],
                                           "%Y-%m-%d %H:%M:%S").month
        year = datetime.datetime.strptime(item['time'],
                                          "%Y-%m-%d %H:%M:%S").year
        if billing_date.year == year:
            new_month(customer_list, month, year)
            billing_date = datetime.datetime.strptime(item["time"],
                                                      "%Y-%m-%d %H:%M:%S")
        elif month != billing_month:
            new_month(customer_list, month, year)
            billing_date = datetime.datetime.strptime(item["time"],
                                                      "%Y-%m-%d %H:%M:%S")
        if item["type"] == "call":
            call = Call(item["src_number"], item["dst_number"],
                        datetime.datetime.strptime(item["time"],
                                                   "%Y-%m-%d %H:%M:%S"),
                        item["duration"], tuple(item["src_loc"]),
                        tuple(item["dst_loc"]))
            call_from = find_customer_by_number(item["src_number"],
                                                customer_list)
            call_from.make_call(call)
            call_to = find_customer_by_number(item["dst_number"], customer_list)
            call_to.receive_call(call)
