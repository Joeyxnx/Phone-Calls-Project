    ...
    def apply(self, customers: list[Customer],
        ...
        for cust in customers:
            try:
                ...
                return data
        if new_customer:
            previous_calls = new_customer.get_history()
            for calls in data:
                if calls in previous_calls[0]:
                    data_lst.append(calls)
                elif calls in previous_calls[1]:
                    data_lst.append(calls)
            return data_lst
        else:
            return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        if len(filter_string) > 0 and filter_string[0].isalpha() and \
            ...
            current = int(filter_string[1:])
            for calls in data:
                if filter_string[0].upper() == 'L' and calls.duration < current:
                    data_lst.append(calls)
                elif filter_string[0].upper() == 'G' and calls.duration > \
                        current:
                    data_lst.append(calls)
            return data_lst
        else:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        data_lst = []
        mapmax = (-79.697878, 43.799568)
        mapmin = (-79.196382, 43.576959)
        try:
            if len(filter_string) > 3 and ',' in filter_string:
                new_lst = filter_string.split(',')
                lower_long = float(new_lst[0])
                lower_lat = float(new_lst[1])
                upper_long = float(new_lst[2])
                upper_lat = float(new_lst[3])
            else:
                return data
        except ValueError:
            return data
        if lower_long < mapmax[0] and lower_lat < mapmin[1] and \
                upper_long > mapmin[0] and upper_lat > mapmax[1]:
            pass
        elif lower_long > mapmax[0] and lower_lat > mapmin[1] and \
                upper_long < mapmin[0] and upper_lat < mapmax[1]:
            for calls in data:
                if lower_long < calls.src_loc[0] < upper_long:
                    if lower_lat < calls.src_loc[1] < upper_lat:
                        data_lst.append(calls)
        else:
            return data
        return data_lst
